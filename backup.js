...
app.post('/addPost', (req, res) => {
    ...
    const query = "INSERT INTO posts (`title`,`shortDesc`, `description`, `image`, `category`, `date`, `userId`) VALUES (?)";
...
//     const query = req.query.category ? 'SELECT * FROM posts WHERE category = ?'
//         : 'SELECT * FROM posts'
...
app.put('/updatePost/:id', (req, res) => {
    const postId = req.params.id;
    const query = 'UPDATE posts SET `title`=?, `description`=?, `image`=?, `category`=?, `shortDesc`=? WHERE id = ?'
    const values = [
        req.body.title,
        req.body.description,
        req.body.image,
